...
class ImageRecognizer:
    ...
    def save_number_image(self, number, processed, mon_id):
        """保存数字图片到对应文件夹
        Args:
            number: 识别出的数字
            processed: 处理后的图片
    ...
    def add_black_border(self, img, border_size=3):
    ...
    def crop_to_min_bounding_rect(self, image):
        """裁剪图像到包含所有轮廓的最小外接矩形"""
    ...
    def find_best_match(self, target, ref_images):
        """
    ...
    def process_regions(self, main_roi, screenshot=None):
        """处理主区域中的所有区域（优化特征匹配）
        Args:
            main_roi: 主要感兴趣区域的坐标
        ...
        Returns:
            区域处理结果的列表
        """
        ref_images = self.load_ref_images()
        results = []
    ...
    def load_ref_images(self, ref_dir=get_config()["ui_photos_direc"]):
        """加载参考图片库"""
        ref_images = {}
        for i in range(56):
    ...
    def recognize(self, screenshot=None):
        """
        对外提供的识别接口
        :param screenshot: 可选参数，如果提供则使用提供的截图，否则会截取屏幕
        :return: 识别结果列表
        """
        print("请用鼠标拖拽选择主区域...")
...
# 提供模块级别的便捷函数
def recognize_monsters(screenshot=None):
    """
    便捷函数：创建识别器并执行识别
    :param screenshot: 可选参数，如果提供则使用提供的截图，否则会截取屏幕
    :return: 识别结果列表
